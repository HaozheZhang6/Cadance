...
\paragraph{Reference baseline is intentionally weak.} Our open BenchCAD-trained baseline (\S\ref{app:baseline}) is a community lower bound, not an attempt at SOTA. We do not apply RL, do not perform multi-task pretraining, and do not exhaust the training corpus.

\paragraph{Edit Leakage measurement.} Our non-target preservation metric uses voxel IoU on the spatial complement of the targeted feature; this captures most leakage modes but can miss small parametric shifts whose voxel footprint is subthreshold. \S\ref{app:edit_protocol} discusses the protocol, including a complementary code-AST diff metric we report alongside.

\section{Transfer Baseline Detail: cadrille vs.\ CADEvolve}
\label{app:transfer_detail}

The CAD-Recode/cadrille/CADEvolve lineage~\citep{Rukhovich2025,Kolodiazhnyi2026,Elistratov2026} represents the current state of the art on existing CAD code-generation benchmarks (DeepCAD image IoU 92.2 for cadrille, 92.6 for CADEvolve; Fusion360 image IoU 84.6 and 87.2 respectively). All three release inference-only checkpoints sharing the Qwen2-VL-2B backbone with multi-view image input compatible with BenchCAD's render protocol.

\paragraph{cadrille vs.\ CADEvolve --- training data.} cadrille fine-tunes on a 1M-script procedural corpus restricted to sketch+extrude+Boolean. CADEvolve extends the corpus to 2.7M scripts via an LLM-driven evolutionary loop whose seed pool covers extrude, revolve, loft, sweep, shell, fillet, chamfer, Boolean operations, and patterns --- a meaningful broadening of the operation distribution but still without helical sweeps or parametric involute-gear construction.

\paragraph{cadrille vs.\ CADEvolve --- BenchCAD transfer.} We evaluate \texttt{maksimko123/cadrille-rl} and \texttt{kulibinai/cadevolve-rl1} on BenchCAD's \texttt{img2cq} task. cadrille loses 32 points on BenchCAD-Lite (92.2 $\to$ 60.5); CADEvolve loses 28 points (92.6 $\to$ 64.1). The 4-point CADEvolve advantage concentrates on families that exercise the additional operations CADEvolve introduced (revolve, loft, sweep, fillet, chamfer, shell, patterns) but not on the standard-anchored hard families (helical gears, twist drills, coil springs) where both models drop equally because both lack the operations these families require. CADEvolve's broader operation coverage helps where it overlaps with BenchCAD's operation set, but does not close the Op Vocabulary Gap on the operations BenchCAD includes that CADEvolve still omits.

\paragraph{Why the gap is honest, not artefactual.} We replicate the lineage's render protocol (8-view 238$\times$238 grid; cube normalisation to $[-100, 100]^3$) for the transfer evaluation, eliminating mismatches in viewing geometry as a confound. We further verify that both checkpoints reach published IoU values on a held-out 200-sample DeepCAD slice within $\pm$0.5\,pt under our scoring infrastructure. The remaining gap on BenchCAD is therefore a true generalisation deficit: training corpora restricted to sketch+extrude (cadrille) or to sketch+extrude+revolve+fillet (CADEvolve) do not transfer to families requiring helical sweeps, parametric gear teeth, or DIN-anchored standard parts.
...
To bound the difficulty of BenchCAD with publicly reproducible numbers, we release a baseline model trained directly on the dataset. We fine-tune Qwen2-VL-2B on a mixture of the 162K-pair BenchCAD-GenCAD corpus and the 17{,}900-record BenchCAD-Synth split (image$\to$code), training for 2 epochs at learning rate $1\times10^{-5}$ on 8$\times$A800-80GB GPUs ($\approx$9 GPU-hours total). We do not apply reinforcement learning; this baseline targets reproducibility, not state of the art. Training script, data manifest, and checkpoint are released under MIT licence. The baseline reaches an aggregate score of 18.4\% on BenchCAD-Lite --- comparable to mid-tier proprietary models and substantially below the same-architecture cadrille-RL release. Its purpose is to provide a community lower bound and to make the transfer-baseline gaps interpretable: any improvement attributable to procedural data scaling, multi-task pre-training, or RL fine-tuning appears as a contrast against this open baseline rather than against an opaque private model.
