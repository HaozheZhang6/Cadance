...
Table~\ref{tab:dataset} reports the dataset composition; Table~\ref{tab:capability} maps each task to its sub-capability subset; Tables~\ref{tab:main_eval_qa_img} and~\ref{tab:main_eval_qa_code} report the per-model QA leaderboard (Vision QA / Code QA); Table~\ref{tab:codegen_unified} is the unified Vision2Code leaderboard (specialist + frontier + ours); Table~\ref{tab:family_cliff} stratifies \texttt{img2cq} by family difficulty.
...
\textbf{Was preprocessing/cleaning done?} Verification (IoU $\geq 0.99$ rotation-invariant against the builder solid; non-degenerate volume; 30\,s timeout) is the sole filter applied to BenchCAD.
