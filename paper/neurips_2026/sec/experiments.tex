...
Table~\ref{tab:main_eval_codegen} reports the Vision2Code task, where two qualitative patterns stand out. 
First, thinking mode is non-monotonic on \texttt{Vision2Code} task. 
Combined with uniformly low Detailed Feature scores, this is consistent with extended reasoning amplifying hallucinations on fine geometric features while the global shape is recovered. 
Second, execution rate decouples from total score: the model with the highest execution rate is not the strongest overall, and a model with notably lower execution rate can still match its total. Frontier models have largely learned to emit syntactically valid CadQuery, so the residual deficit is geometric rather than syntactic, and codegen progress should be tracked through IoU and Essential Op Recall rather than execution rate.
...
Building on the layered evaluation introduced in \S\ref{sec:capability}, we examine four representative \textsc{Vision2Code} failures elicited from frontier reasoning models on hard-tier families, each diagnostic of a different level in the capability hierarchy (visualised in Fig.~\ref{fig:capability_profile}).
...
\subsection{Training: Open BenchCAD Baseline + RL Fine-Tuning}
\label{sec:exp_training}

We train Qwen3-VL-2B baselines under matched 32 GPU-hour budgets on A800-80GB.
\textbf{SFT}: 1 epoch, lr $1{\times}10^{-5}$, 300K mixture (BenchCAD-Synth + 162K GenCAD + CAD-Recode/text2cad); three splits --- \emph{IID} (all 106 families), \emph{baseline} (no BenchCAD), \emph{OOD} (10 mech.-transmission families held out).
\textbf{RL}: on-policy GRPO from the OOD-SFT checkpoint, reward $r{=}0.2\,\mathrm{ess}{+}0.8\,\mathrm{IoU}$ ($r{=}-1$ on parse error / timeout / zero-volume); RL data excludes OOD families; no warm-start.
End-of-training metrics (Table~\ref{tab:training_compact}, Appendix~\ref{app:op_gap_full}): IID converges to IoU $\sim$0.65, ess $\sim$0.95; the baseline plateaus at IoU 0.10 with \textbf{0\% essential-op pass}; RL on OOD-SFT lifts OOD IoU 0.18$\to$0.35 and ess 0.20$\to$0.45 but the residual gap to IID is the empirical \emph{generalisation cliff}.
