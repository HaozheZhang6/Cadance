...
\paragraph{Metrics.} For each prediction we report the execution rate \texttt{exec\_pct}, mean voxel IoU between rendered prediction and target, mean Chamfer distance \texttt{mean\_cd}, an \emph{essential-op} recall \texttt{ess\_score} (fraction of expected operation tokens recovered in the emitted code), and a composite \texttt{total} score (rendered as the rightmost column of every leaderboard). All metrics are pass@1: one API call per (record, model) with no resampling. A complementary 6-orientation rotation-invariant voxel IoU is computed alongside the standard IoU and used only for failure attribution (\S\ref{sec:exp_failure_analysis}, \emph{wrong-plane} diagnostic).
