...
\textbf{Industrial Common Sense Gap} is the failure mode where a model applies the requested change correctly but silently corrupts an unrelated parametric feature (enlarging a gear bore while accidentally changing the tooth count, adding a chamfer while shifting a hole's centre).
Disaggregating Pass@1 reveals that even the strongest reasoning model achieves target success on 84\% of edits but full Pass@1 on only 31\%: \textbf{64\% of nominally successful edits exhibit Industrial Common Sense Gap} --- a locality failure invisible to any prior CAD eval that scores only output IoU.
...
\textbf{Holistic Spatial Deficit on edit}: the \texttt{edit\_img}/\texttt{edit\_code} contrast reproduces the Holistic Spatial and Detailing Deficit pattern (GPT-5.3: 52.4\% text-only vs.\ 31.0\% image-conditioned, a 21.4\,pt gap).
