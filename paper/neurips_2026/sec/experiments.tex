...
Table~\ref{tab:codegen_unified} unifies the Vision2Code leaderboard across three model classes: specialist CAD lineage (cadrille / CADEvolve, transferred from DeepCAD), frontier MLLMs (BenchCAD-Lite, OOD by definition), and our open Qwen3-VL-2B baseline (SFT-IID / SFT-OOD / $+$RL).
Three patterns: (i) the specialist lineage drops from $\sim$92\% IoU on DeepCAD to $\leq$0.273 on BenchCAD --- a 28--32\,pt collapse driven by missing helical sweeps and parametric gear teeth;
(ii) frontier MLLMs cap at IoU 0.279 (gemini-3.1-pro-thinking), with thinking mode non-monotonic and execution rate decoupled from quality (the residual deficit is geometric, not syntactic);
(iii) our Qwen3-VL-2B reaches IoU 0.65 on the IID split (the dataset is learnable on a small backbone), yet collapses to 0.18 on 10 held-out mech.-transmission families, which RL on the OOD-SFT checkpoint partially recovers to 0.35 --- the empirical \emph{generalisation cliff}.
...
For reproducibility we describe the open Qwen3-VL-2B baseline whose scores appear in the bottom block of Table~\ref{tab:codegen_unified}. Matched 32 GPU-hour budget on $8{\times}$A800-80GB.
\textbf{SFT}: 1 epoch, lr $1{\times}10^{-5}$, 300K mixture (BenchCAD-Synth + 162K GenCAD + CAD-Recode / Text2CAD), three splits --- IID (106 families seen), OOD (10 mech.-transmission families held out), baseline (no BenchCAD).
\textbf{RL}: on-policy GRPO from the OOD-SFT checkpoint, reward $r{=}0.2\,\mathrm{ess}{+}0.8\,\mathrm{IoU}$ ($r{=}-1$ on parse error / $>$30\,s timeout / zero-volume); RL data excludes OOD families; no warm-start. Training curves in Appendix~\ref{app:op_gap_full} (Fig.~\ref{fig:generalisation_gap}); per-operation recall ablation in Table~\ref{tab:op_gap}.
