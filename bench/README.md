...
框架 sanity check：跑 100 条**人写的**（非 synth）fusion360 / deepcad verified 样本，看 image→code 的 IoU/F1 是否合理。只 code bench 适用（外部数据无 qa_pairs）。

**Step 1 — 上传**（在 workspace 跑，需要 `verified_parts.csv` + VTK）：
...
从 `data/data_generation/verified_parts.csv` 抽各 50 条（`gt_norm_step_path` + `norm_cq_code_path` 都齐的），cadrille renderer 生成 composite.png，加 `feature_tags`，推 HF。

**Step 2 — 跑 bench**（任意机器，不需要 VTK）：
...
IoU / Chamfer / Feature-F1 计算前都走 `_load_normalized_mesh`（bbox center→`[0.5,0.5,0.5]`，longest→`[0,1]³`），外部数据尺寸无关。
