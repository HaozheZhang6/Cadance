"""UA-20 — Finalize pairs_curated.jsonl from 3 plans:
  - curate_final_plan.json (dim edits, L1+L2)
...
def emit_dim_records(plan: dict, exclude_hard: bool, exclude_iou_broken: bool):
    ...
    for fam, e in sorted(plan.items()):
...
def emit_additive_records(plan: dict):
    """L1 only per family (natural-language 'add feature' instruction)."""
    records = []
    for name, e in sorted(plan.items()):
        fam = e["family"]
        op = e["op_type"].replace("add_", "")
        rid = f"curated_{fam}_add_{op}_L1"
        records.append({
            "record_id": rid,
            "family": fam,
            "edit_type": e["op_type"],
...
def emit_multi_records(plan: dict):
    ...
    for name, e in sorted(plan.items()):
        ...
        l1_text = (f"Set the {human_for(ax1['axis'])} to {t1} mm and "
                   f"the {human_for(ax2['axis'])} to {t2} mm.")
        sign1 = "+" if ax1["pct"] > 0 else ""
        sign2 = "+" if ax2["pct"] > 0 else ""
        l2_text = (f"Change the {human_for(ax1['axis'])} by {sign1}{ax1['pct']}% and "
...
def main():
    ...
    dim_records, dropped = emit_dim_records(dim_plan, args.exclude_hard, args.exclude_iou_broken)
