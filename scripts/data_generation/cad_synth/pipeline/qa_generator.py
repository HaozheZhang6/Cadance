...
Design rule:
  ...
  Dimensional questions are unit-free: the answer uses the same scale as
  the geometry, and the grader compares magnitude.
"""
...
def _hex_nut(p: dict):
    # ISO 4032: nominal_size = M, across_flats = s, bore_diameter = M
    M = p.get("nominal_size", p.get("bore_diameter", p.get("thread_diameter", 10.0)))
    s = p.get("across_flats", round(M * 1.7, 1))
...
def _hex_standoff(p: dict):
    af = p.get("across_flats", p.get("outer_diameter", 10.0))
    h = p.get("height", p.get("length", 20.0))
    m = p.get("m_size", 0)
    qa = [
...
def _gridfinity_bin(p: dict):
    ...
    qa = [
        ...
        _q("How many height units stacked along Z?", uz, "integer"),
    ]
    return qa, {}
