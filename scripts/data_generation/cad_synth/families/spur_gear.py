...
class SpurGearFamily(BaseFamily):
    ...
    def sample_params(self, difficulty: str, rng) -> dict:
        ...
        _ISO54 = [0.8, 1.0, 1.25, 1.5, 2.0, 2.5, 3.0, 4.0,
                  0.9, 1.125, 1.375, 1.75, 2.25, 2.75, 3.5]
        ...
        if variant == "spoked":
            ...
            hub_r = round(rng.uniform(bore_d / 2 + 2, max(bore_d / 2 + 3, r_p * 0.3)), 1)
            ...
            params.update(n_spokes=n_spokes, hub_radius=hub_r,
                          spoke_width=spoke_w, inner_void_radius=inner_r)
            ...
            hub_r = round(rng.uniform(bore_d / 2 + 2, max(bore_d / 2 + 3, hub_r_max)), 1)
            ...
            params.update(web_radius=web_r, web_height=web_h, hub_radius=hub_r,
                          web_recess_sides=web_sides, web_recess_side=str(web_side))
            ...
            hub_r = round(rng.uniform(bore_d / 2 + 2, max(bore_d / 2 + 3, mid_r * 0.55)), 1)
            ...
            params.update(mid_ring_radius=mid_r, hub_radius=hub_r,
                          web1_height=web1_h, web2_height=web2_h)
        ...
        if _add_web_recess:
            ...
            side = rng.choice(["<Z", ">Z"])
            params.update(web_recess_radius=wcr, web_recess_depth=wrd,
        ...
        if difficulty in ("medium", "hard"):
            key_w = round(rng.uniform(bore_d * 0.2, max(bore_d * 0.21, bore_d * 0.3)), 1)
        ...
        if difficulty == "hard" and variant == "solid_disc":
            if _add_web_recess:
                ...
                light_pcd = round(rng.uniform(l_pcd_min, max(l_pcd_min + 0.1, r_p * 0.75)), 1)
                params.update(n_lightening=n_light, lightening_radius=light_r,
            ...
            params["chamfer_length"] = round(rng.uniform(0.3, max(0.31, min(1.0, m * 0.3))), 1)
    ...
    def make_program(self, params: dict) -> Program:
        ...
        if variant == "solid_disc":
            ...
            spoke_cx  = round((hub_r + inner_r) / 2, 3)    # radial midpoint for box centre [mm]
            for i in range(n_sp):
                angle = round(360.0 * i / n_sp, 3)  # rotation angle for this spoke [degrees]
                ops.append(Op("union", {"ops": [
                    ...
                    {"name": "transformed", "args": {
                        "offset": [spoke_cx, 0, round(fw / 2, 3)],  # shift radially + to mid-height
                        "rotate": [0, 0, 0],
                    }},
                    {"name": "box", "args": {
            ...
            if recess > 0.5:  # skip trivially shallow recesses
                for face in faces:
                    ...
                    ops.append(Op("circle", {"radius": web_r}))          # outer boundary of annulus
                    ops.append(Op("circle", {"radius": inner_r_cut}))    # inner boundary (clears keyway)
                    ops.append(Op("cutBlind", {"depth": recess}))         # builder auto-negates
            ...
            mid_r = params["mid_ring_radius"]   # intermediate concentric ring radius [mm]
            hub_r = params["hub_radius"]         # hub outer radius [mm]
            ...
            ops.append(Op("union", {"ops": [
                ...
                {"name": "cylinder", "args": {"height": fw, "radius": mid_r}},
            ]}))
        ...
        if wcr and wrd:
            ...
            for side in sides:
                ...
                ops.append(Op("circle", {"radius": bore_r})) # inner annulus boundary (preserved)
                ops.append(Op("cutBlind", {"depth": round(wrd, 3)}))  # builder auto-negates
        ...
        if rb_h and variant == "solid_disc":
            ...
            ops.append(Op("union", {"ops": [
                ...
                {"name": "cylinder", "args": {"height": round(rb_h, 3), "radius": rim_r}},
            ]}))
            ...
            if hb_r:
                ...
                ops.append(Op("union", {"ops": [
                    ...
                    {"name": "cylinder", "args": {"height": round(rb_h, 3), "radius": round(hb_r, 3)}},
                ]}))
