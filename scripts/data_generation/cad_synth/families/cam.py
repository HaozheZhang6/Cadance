...
class CamFamily(BaseFamily):
    ...
    def sample_params(self, difficulty: str, rng) -> dict:
        ...
        if difficulty in ("medium", "hard"):
            lobe_h = round(rng.uniform(r_base * 0.1, max(r_base * 0.11, r_base * 0.35)), 1)
            lobe_angle = round(rng.uniform(0, 360), 1)
            params["lobe_height"] = lobe_h
            params["lobe_angle"] = lobe_angle
            kw = round(max(1.5, rng.uniform(bore_d * 0.30, max(bore_d * 0.31, bore_d * 0.5))), 1)
            params["keyway_width"] = kw

        if difficulty == "hard":
            # Second lobe at 180°
            lobe2_h = round(rng.uniform(
            ...
            params["oil_hole_diameter"] = round(rng.uniform(1.5, max(1.6, min(5.0, bore_d * 0.4))), 1)
    ...
    def make_program(self, params: dict) -> Program:
        ...
        if lh:
            ...
            if lh2:
                ...
                for i in range(n):
                    ...
                    r = rb + b1 + b2
                    final_pts.append((round(r * math.cos(a), 3), round(r * math.sin(a), 3)))
                pts = final_pts
            else:
        ...
        if ohd:
            ops.append(Op("cut", {"ops": [
                {"name": "transformed", "args": {
                    "offset": [round(rb * 0.5, 3), 0.0, round(thick / 2, 3)],
                    "rotate": [0, 0, 0],
                }},
                {"name": "cylinder", "args": {
