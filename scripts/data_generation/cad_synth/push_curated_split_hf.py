...
def write_shards_and_upload(rows, out_dir: Path, repo: str, token: str,
                             shard_size: int = 2000, private: bool = False):
...
def main():
    ...
    token = (os.environ.get("BenchCAD_HF_TOKEN")
    ...
    for i in range(len(bench)):
        r = bench[i]
        bench_fdp[(r["family"], r["difficulty"], r.get("base_plane", "XY"))].append(r["stem"])
    for k in bench_fdp:
        bench_fdp[k].sort()
    ...
    print(f"  substitutes matched: {len(substitutions)} / "
    ...
    if not args.skip1:
        print(f"\n=== Push 1: {args.repo1} ===")
        write_shards_and_upload(ds1_rows, Path(args.shard_dir1), args.repo1, token,
                                args.shard_size)
    else:
        print(f"\n[skip1] Dataset 1 push skipped")
    if not args.skip2:
        print(f"\n=== Push 2: {args.repo2} ===")
        write_shards_and_upload(ds2_rows, Path(args.shard_dir2), args.repo2, token,
                                args.shard_size)
    else:
